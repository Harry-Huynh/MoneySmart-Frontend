...
function getSavingGoalToastUI(milestoneKey, notificationLevel) {
  if (notificationLevel === "SUCCESS" || milestoneKey === "ACHIEVED") {
    return { icon: <FiCheckCircle /> };
  }

  switch (milestoneKey) {
    case "EXCEEDED":
      return { icon: <FaBullseye /> };
    case "P80":
      return { icon: <FaBullseye /> };
    case "P50":
...
export async function createSavingGoalNotificationDataByMilestones(
  ...
  const savingGoal =
    savingGoalResponse.savingGoal || savingGoalResponse.goal || savingGoalResponse;
  ...
  if (progressRatioBeforeTransaction >= 1 && transactionAmount > 0) {
    return {
      ...
      message: `You added ${formatMoneyCAD(transactionAmount)} after reaching the target ${formatMoneyCAD(targetAmount)} for "${savingGoalName}".`,
  ...
  if (progressRatioAfterTransaction >= 1 && progressRatioBeforeTransaction < 1) {
    const exceededAmount = Math.max(0, savedAmountAfterTransaction - targetAmount);

    return {
      type: NOTIFICATION_TYPE,
      title:
        exceededAmount > 0 ? "Goal Achieved (Exceeded)" : "Saving Goal Achieved",
      message:
        exceededAmount > 0
          ? `You achieved "${savingGoalName}" and exceeded by ${formatMoneyCAD(exceededAmount)}.`
  ...
  if (highestCrossedMilestone) {
    return {
      ...
      message: `You're at ${highestCrossedMilestone.label} for "${savingGoalName}" (${formatMoneyCAD(savedAmountAfterTransaction)} saved of ${formatMoneyCAD(targetAmount)}).`,
      deliveryMethod: DELIVERY_METHOD,
      level: "INFO",
